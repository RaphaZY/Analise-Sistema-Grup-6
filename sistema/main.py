...
class Sistema:
    ...
    def menu_principal(self):
        while True:
            ...
            if not self.usuario_logado:
                ...
                print(f"Bem-vindo, {self.usuario_logado.get_nome()}!")
                if self.usuario_logado.get_tipo() == 'cliente':
    ...
    def cadastrar_usuario(self):
        ...
        email = input("Email: ")
        senha = input("Senha: ")
        tipo = input("Tipo (cliente/prestador): ")
        
        # Verificar se CPF já existe
        if any(u['cpf'] == cpf for u in self.db.buscar_todos('usuarios')):
            print("CPF já cadastrado!")
            self.pausar()
            return
        
        # Verificar se email já existe
        if any(u['email'] == email for u in self.db.buscar_todos('usuarios')):
        ...
        # Criar usuário
        usuario = Usuario(nome, cpf, email, senha, tipo)
        usuario.set_biometria_hash(biometria_hash)
        
        self.db.inserir('usuarios', usuario)
        ...
        self.db.inserir('notificacoes', notificacao)
    ...
    def login_cpf_senha(self):
        ...
        usuarios = [Usuario.from_dict(u) for u in self.db.buscar_todos('usuarios')]
        for usuario in usuarios:
            if usuario.get_cpf() == cpf and usuario.autenticar_senha(senha):
                ...
                notificacao = Notificacao(
                    usuario.get_id(),
                    "Login realizado",
                    f"Login realizado com sucesso em sua conta."
                )
                self.db.inserir('notificacoes', notificacao)
    ...
    def login_biometria(self):
        ...
        for usuario in usuarios:
            if usuario.get_cpf() == cpf:
                if usuario._biometria_hash is None:
                ...
                if usuario.autenticar_biometria(biometria_hash):
                    ...
                    notificacao = Notificacao(
                        usuario.get_id(),
                        "Login por biometria",
                        f"Login realizado com sucesso usando biometria facial."
                    )
                    self.db.inserir('notificacoes', notificacao)
                    
                    print("✓ Biometria reconhecida! Login realizado!")
                    self.pausar()
                    return
                else:
    ...
    def configurar_biometria(self):
        ...
        dados_biometricos = f"{self.usuario_logado.get_cpf()}_{self.usuario_logado.get_nome()}_biometria_facial_nova"
        biometria_hash = hashlib.sha256(dados_biometricos.encode()).hexdigest()
        
        # Configurar biometria para o usuário
        self.usuario_logado.set_biometria_hash(biometria_hash)
        
        # Atualizar no banco
        for i, usuario_dict in enumerate(self.db.dados['usuarios']):
            if usuario_dict['id'] == self.usuario_logado.get_id():
                self.db.dados['usuarios'][i] = self.usuario_logado.to_dict()
                self.db.salvar()
                break
        
        # Notificação
        notificacao = Notificacao(
            self.usuario_logado.get_id(),
    ...
    # (buscar_servicos, solicitar_servico, meus_contratos, etc.)
    
    def buscar_servicos(self):
        print("\n--- SERVIÇOS DISPONÍVEIS ---")
        servicos = [Servico.from_dict(s) for s in self.db.buscar_todos('servicos')]
        ...
        for i, servico in enumerate(servicos_ativos, 1):
            prestador_dict = next((u for u in self.db.buscar_todos('usuarios') 
                           if u['id'] == servico.get_prestador_id()), None)
            if prestador_dict:
                prestador = Usuario.from_dict(prestador_dict)
                
                # Calcular avaliação média do prestador
                avaliacoes = [Avaliacao.from_dict(a) for a in self.db.buscar_todos('avaliacoes')]
    ...
    def solicitar_servico(self, servico):
        ...
        if confirmar.lower() == 's':
            ...
            self.db.inserir('contratos', contrato)
            
            # Criar pagamento
            pagamento = Pagamento(contrato.get_id(), servico.get_valor())
            self.db.inserir('pagamentos', pagamento)
            ...
            self.db.inserir('notificacoes', notificacao_cliente)
    ...
    def meus_contratos(self):
        print("\n--- MEUS CONTRATOS ---")
        contratos = [Contrato.from_dict(c) for c in self.db.buscar_todos('contratos')]
        meus_contratos = [c for c in contratos if c.get_cliente_id() == self.usuario_logado.get_id()]
    ...
    def detalhes_contrato_cliente(self, contrato):
        ...
        pagamentos = [Pagamento.from_dict(p) for p in self.db.buscar_todos('pagamentos')]
    ...
    def avaliar_servico(self, contrato):
        ...
        if prestador_dict:
            ...
            self.db.inserir('avaliacoes', avaliacao)
    ...
    def minhas_avaliacoes(self):
        ...
        if self.usuario_logado.get_tipo() == 'cliente':
            minhas_avaliacoes = [a for a in avaliacoes if a.get_avaliador_id() == self.usuario_logado.get_id()]
            print("Avaliações que você fez:")
        else:
            minhas_avaliacoes = [a for a in avaliacoes if a.get_avaliado_id() == self.usuario_logado.get_id()]
        ...
        for i, avaliacao in enumerate(minhas_avaliacoes, 1):
            usuario_dict = next((u for u in self.db.buscar_todos('usuarios') 
                        if u['id'] == (avaliacao.get_avaliador_id() if self.usuario_logado.get_tipo() == 'prestador' else avaliacao.get_avaliado_id())), None)
    ...
    def meus_pagamentos(self):
        ...
        if self.usuario_logado.get_tipo() == 'cliente':
            ...
            meus_contratos = [c for c in contratos if c.get_prestador_id() == self.usuario_logado.get_id()]
    ...
    def ver_notificacoes(self):
        print("\n--- NOTIFICAÇÕES ---")
        notificacoes = [Notificacao.from_dict(n) for n in self.db.buscar_todos('notificacoes')]
        minhas_notificacoes = [n for n in notificacoes if n.get_usuario_id() == self.usuario_logado.get_id()]
        ...
        if opcao == '1':
            for notificacao in nao_lidas:
                ...
                self.db.atualizar('notificacoes', notificacao.get_id(), notificacao.to_dict())
    ...
    def meus_servicos(self):
        print("\n--- MEUS SERVIÇOS ---")
        servicos = [Servico.from_dict(s) for s in self.db.buscar_todos('servicos')]
        meus_servicos = [s for s in servicos if s.get_prestador_id() == self.usuario_logado.get_id()]
            ...
            if meus_servicos:
                ...
                if num.isdigit():
                    ...
                    if 1 <= num <= len(meus_servicos):
                        ...
                        self.db.atualizar('servicos', servico.get_id(), servico.to_dict())
        self.pausar()
    ...
    def cadastrar_servico(self):
        ...
        servico = Servico(self.usuario_logado.get_id(), titulo, descricao, valor)
        self.db.inserir('servicos', servico)
    ...
    def solicitacoes_servico(self):
        print("\n--- SOLICITAÇÕES ---")
        contratos = [Contrato.from_dict(c) for c in self.db.buscar_todos('contratos')]
        solicitacoes = [c for c in contratos if c.get_prestador_id() == self.usuario_logado.get_id()]
        ...
        for i, contrato in enumerate(solicitacoes, 1):
            cliente_dict = next((u for u in self.db.buscar_todos('usuarios') 
    ...
    def detalhes_contrato_prestador(self, contrato):
        ...
        if contrato.get_status() == "solicitado":
            if opcao == '1':
                ...
                if pagamento_contrato:
                    ...
                    self.db.atualizar('pagamentos', pagamento_contrato.get_id(), pagamento_contrato.to_dict())
        ...
        self.db.atualizar('contratos', contrato.get_id(), contrato.to_dict())
        self.pausar()
    
    def editar_perfil(self):
        print("\n--- EDITAR PERFIL ---")
        print(f"Nome atual: {self.usuario_logado.get_nome()}")
        print(f"Email atual: {self.usuario_logado.get_email()}")
        print(f"CPF atual: {self.usuario_logado.get_cpf()}")
        ...
        if nova_senha:
            self.usuario_logado.set_senha(nova_senha)
...
if __name__ == "__main__":
    ...
    if not sistema.db.buscar_todos('usuarios'):
        print("Criando dados iniciais...")
        
        # Prestadores
        prestador1 = Usuario("João Silva", "12345678901", "joao@email.com", "123", "prestador")
        # Configurar biometria para usuários iniciais
        ...
        sistema.db.inserir('usuarios', prestador1)
        
        prestador2 = Usuario("Maria Santos", "98765432100", "maria@email.com", "123", "prestador")
        ...
        sistema.db.inserir('usuarios', cliente1)
        
        cliente2 = Usuario("Ana Costa", "55566677788", "ana@email.com", "123", "cliente")
        ...
        sistema.db.inserir('usuarios', cliente2)
